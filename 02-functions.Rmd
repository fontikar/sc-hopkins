...
### Exercise/Discussion: return()

- What is the `return()` function doing? What happens when you remove it and use the function again? 
...
# Put the solution in the function, embrace {{var}}
count_n_obs <- function(data, var){
  data |> 
  group_by({{var}}) |> 
  summarise(n_obs = n())
...
- ChatGPT is pretty good too at debugging
...
Debug and fix the following function with any of the approaches above.

There are 2 errors here

```{r}
species = cleaned_af$species


create_underscore_species_name <- function(species){
  genus <- stringr::word(species,1)
  
  intraspec_epithet <- stringr::word(species,3)
  
  paste0(genus, intraspec_epithet, sep = "_")
}
```
