---
title: "Scaling up your computations"
date: "`r Sys.Date()`"
format: html
---
...
penguins_combined_with_common_names |>
  # use select to sepevify the order
  select(species, common_name, where(is_character), everything())
...
penguins_combined$individualCount[is.na(penguins_combined$individualCount)] <- 0
...
```{r, learner-solutions}
penguins_combined$species |> unique() |> length()

data_counts <- 
  penguins_combined|>
  mutate(
    individualCount = ifelse(is.na(individualCount), 1, individualCount) |> as.numeric()
  ) |>
  group_by(species) |>
  summarise(
    n_record = n(),
    n_counts = sum(individualCount)
    )

data_counts |> 
  arrange(desc(n_record)) |>
  slice(1:3)
...
size_AVG <-
  ...
  group_by(species) %>% 
  summarise(
    ave_bill_length = mean(bill_length_mm, na.rm=TRUE),
    ave_flipper_length = mean(flipper_length_mm, na.rm=TRUE)    
  ) %>%
  mutate(
    species = ifelse(species == "Adelie", "Psygoscelis adealie", species)
  )

size_AVG$species[size_AVG$species == "Adelie"] <- "Psygoscelis adealie"
...
years_files <- paste("output/", years, "_penguins.csv", sep = "")

map2(data_split, years_files, write_csv)

for(i in seq_len(length(years))) {
  write_csv(data_split[[i]], years_files[[i]])
}


out <- list()
for(i in seq_len(length(years))) {
  out[[i]] <- nrow(data_split[[i]])
}
