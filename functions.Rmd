...
- Extend your function. Add an input variable that specifies whether a boxplot or a violin plot will be produced. You may need to call a function within your new function. 
...
quick_viz<- function(data, y_var, x_var, type){
  ...
  # browser()
  if(type == "boxplot"){
    quick_boxplot(data, y_var = y_var, x_var= x_var)
  }
  
 if(type == "violin"){
   quick_violin(data = data, y_var = y_var, x_var = x_var)
 }
}

quick_viz(cleaned_ap, individual_count, species, type = "violin")

quick_boxplot(cleaned_ap, individual_count, species)
quick_violin(cleaned_ap, individual_count, species)
```
...
The main functions from purrr you will likely use is `map_` and its variants `map2`, `imap` or `walk_` and its variants. 

`map` loves the split-apply-combine problems. It is written for exactly this purpose.

Let me demonstrate first. Look inside the `data/` folder. There you can see
