...
- Can you think of a use case of `map` in your work? 
...
```{r, instructor-purrr-e2}
split(penguins_combined, ~year) |> 
map(~ glimpse(.x))
```
...
Try extract the R squared from the models we just fitted. You may need `summary()` and `str()`. 

```{r, learners-purrr-e3}

```

```{r, instructor-purrr-e3}
# Try it first with one output 
summary(fit_one_spp)$r.squared

data_counts |> 
  split(~species) |>
  map(fit_model) |> 
  map_df(~summary(.x)$r.squared) 
...
```{r}
penguins |> 
  mutate(mass_adjusted_bill_length = map2_dbl(bill_length_mm, body_mass_g, ~ .x/.y)) |> 
  relocate(mass_adjusted_bill_length, .after = bill_length_mm) # Move new variable in a nice places
```
...
We are going to create a list column, where the column of the dataframe is going hold a list in each cell using `tidyr::nest()`. Here, I am nesting or tucking in the data for each `species` of penguins from the `palmerpenguins` dataset coordinates of each species into their own tibble ad storing it into a column called `data`. Check it out: 
...
This is essentially splitting the dataframe by species which we've done a few things alreadyy, but isn't it much neater?! We can see that there are 3 species in total and how many observations we have for each at a glance. 

```{r}
nested_penguins
```

Let's have a peek at some data for Adelies. 
...
The benefit of using list columns is to that it works SO well with `purrr` functions, especially map. They go hand in hand, especially if you want to do several operations at the same "grouping variable" that you have nested at. 
...
This is essentially splitting the dataframe by species as we did before but isn't it much neater?! We can see that there are 19 species in total and how many observations we have for each at a glance. 
...
For example, let's say I want to calculate the range size of each penguin. One way I can do this is by drawing alpha-hulls (a form of spatial polygon) using the coordinates data. 
...
# Using safely to handle errors
safe_log <- safely(log)
results <- map(c(1, 10, -1, 100), safe_log)
results

Exercise:
  
  Use safely to handle errors in a custom function that divides numbers.
7. Combining purrr with dplyr
Example:
