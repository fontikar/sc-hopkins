...
# With broom R package
data_counts |> 
  split(~species) |>
  map(fit_model) |> 
  map_df(~broom::glance(.x)$r.squared) 
...
# Walking instead of mapping for plots

`map` functions has a cousin called `walk` that essentially does the same thing...but it returns the output **invisibly**, its typically used in conjunction with ggplots

Here I've written a function to plot body mass by bill length

```{r}
# Function to create and save ggplot for each species
...
# Try it out on a single species first
penguins |> 
  filter(species == "Gentoo") |> 
  create_plot("Gentoo")

# Use purrr::walk to create and save plots for each species
penguins |>
  split(~species) |>
  walk(~{
    species <- unique(.x$species)
    plot <- create_plot(.x, species)
    ggsave(filename = paste0("output/plot_", species, ".png"), plot = plot)
  }
  )
```
...
```{r, instructors-purrr-e4}
# install.packages(brooms)

nested_penguins <- penguins |>
  group_by(species) |>
  nest()

models <- nested_penguins |> 
  mutate(lm_model = map(data, ~lm(bill_length_mm ~ body_mass_g, data = .x))) 


# Predict body mass using the fitted models
predictions <- models |>
  mutate(predicted = map2(lm_model, data, ~ augment(.x, newdata = .y)))
